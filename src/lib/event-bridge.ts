...
export class AwsEventBridge implements IMessageBroker {
  ...
  sendMessage(messageDetail: IRequestMessage): Promise<IResponseMessage> {
    return this.sendMessages({ Messages: [messageDetail] }).then((res) => {
  ...
  async createEventBridge(Name: string): Promise<any> {
    try {
      return await this.eventBridge.createEventBus({ Name }).promise();
    } catch (err) {
      console.log(err);
    }
  }
